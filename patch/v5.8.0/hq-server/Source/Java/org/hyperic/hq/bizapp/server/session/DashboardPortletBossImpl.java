...
public class DashboardPortletBossImpl implements DashboardPortletBoss {
    ...
    @Transactional(readOnly=true)
    public List<Map<String, Object>> getFormatedMeasurementData(AuthzSubject subj, Integer resId, Integer mtid, AppdefEntityTypeID ctype,
                                        long begin, long end) throws PermissionException {
        List<Map<String, Object>> result = new ArrayList<Map<String,Object>>();
        ...
        try {
            ...
            if (!metrics.isEmpty()) {
                Measurement measurement = metrics.get(0);
                units = measurement.getTemplate().getUnits();
                chartData.put("measurementName", measurement.getTemplate().getName());
                chartData.put("measurementUnits", measurement.getTemplate().getUnits());
            }
            ...
            int unitForUnit = UnitsConvert.getUnitForUnit(units);
            if(UnitsConstants.UNIT_BYTES==unitForUnit||UnitsConstants.UNIT_PERCENT==unitForUnit||UnitsConstants.UNIT_PERCENTAGE==unitForUnit){
            	List<Double> list = new ArrayList<Double>();
            	for (HighLowMetricValue highLowMetricValue : data) {
            		double val = highLowMetricValue.getValue();
            		if (Double.isNaN(val) || Double.isInfinite(val)) {
                        continue;
                    }
            		list.add(val);
				}
            	double[] metricValuesArray = new double[list.size()];
            	for(int i=0;i<list.size();i++){
            		metricValuesArray[i] = list.get(i).doubleValue();
            	}
            	FormattedNumber[] formattedNumbers = UnitsConvert.convertSame(metricValuesArray,units, Locale.US);
            	int i = 0;
            	for (HighLowMetricValue pt : data) {
            		double val = pt.getValue();
            		if (Double.isNaN(val) || Double.isInfinite(val)) {
            		...
            		List<Double> metricValues = new ArrayList<Double>();
            		FormattedNumber formattedNumber = formattedNumbers[i];
            		double value = Double.parseDouble(formattedNumber.getValue().replaceAll(",", ""));
            		units = formattedNumber.getTag();
            	...
            	chartData.put("measurementUnits", units);
            }else {
            	for (HighLowMetricValue pt : data) {
                    List<Double> metricValues = new ArrayList<Double>();
                    double val = pt.getValue();
