    		...
    		<c:choose>
      			<c:when test="${empty pageSizeMenuDisabled}">
        			...
        			<%
                      String paramPs = null ;
                      try{
                           paramPs = (String)pageContext.getAttribute("pageSizeParam");
                           if(paramPs==null){
                               paramPs = (String)request.getAttribute("pageSizeParam");
                               if(paramPs==null){
                                   paramPs = (String)session.getAttribute("pageSizeParam");
                               }
                           }
                           }catch(Exception e){
                            out.println(e);
        			  ...
        			  String psNumber = "15";
        			  if(paramPs!=null)
        			  if(request.getParameter(paramPs)!=null){
        			   psNumber = request.getParameter(paramPs);
        			  }
        			%>
        			
        				<html:select property="${pageSizeParam}" size="1" onchange="goToSelectLocation(this, '${pageSizeParam}',  '${pageSizeAction}');" value="<%=psNumber%>">
