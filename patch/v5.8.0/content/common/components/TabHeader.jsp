...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
  <tr>
    ...
    <td class="BlockTitle">
      <html:link href="${rssUrl}" onfocus="focusOn(this);" onblur="focusOff(this);"><html:img titleKey="dash.home.RSSSubscribe" border="0" page="/images/icon_feed.gif"/></html:link>
    </td>
    </c:if>
...
<c:if test="${not empty subTitle}">
  <span id="<c:out value="${subTitle}_span${adminToken}"/>" class="BlockSubTitle"><c:out value="${subTitle}"/></span>
    ...
    <td class="BlockTitle" align="right">
      <c:choose>
        <c:when test="${not empty adminToken}">
          <html:link page="${adminUrl}" paramId="token" paramName="adminToken" titleKey="dash.home.DisplayCategory.toolbar.configure" onfocus="focusOn(this);" onblur="focusOff(this);"><img src="<html:rewrite page="/images/4.0/icons/properties.gif" />" width="16" height="16" border="0"/></html:link>
        </c:when>
        <c:otherwise>
          <html:link page="${adminUrl}" titleKey="dash.home.DisplayCategory.toolbar.configure" onfocus="focusOn(this);" onblur="focusOff(this);"><img src="<html:rewrite page="/images/4.0/icons/properties.gif" />" width="16" height="16" border="0" /></html:link>
        </c:otherwise>
      </c:choose>
    </td>
    </c:if>
    <c:if test="${not empty portletName}">
      <td class="BlockTitle" align="right">
         ...
         <c:when test='${enableDelete eq "true"}'>
            <a title="<fmt:message key="dash.home.DisplayCategory.toolbar.close"/>" href="javascript:removePortlet(<c:out value="'${portletName}', '${title}'" escapeXml="false"/>)" onfocus="focusOn(this);" onblur="focusOff(this);">
            <html:img page="/images/4.0/icons/cross.gif" width="16" height="16" border="0"/>
            </a>
         </c:when>
         <c:otherwise>
    ...
    <td class="BlockTitle" align="right">
      <a title="<fmt:message key="dash.home.DisplayCategory.toolbar.close"/>" href="javascript:cancelAdvanced()"><html:img page="/images/4.0/icons/cross.gif" border="0"/></a>
    </td>
    </c:if>
  </tr>
</table>
