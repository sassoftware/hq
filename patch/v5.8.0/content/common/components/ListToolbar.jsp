...
<jsu:script>
	delResConfirm = '<fmt:message key='resoure.delete.confirm'/>';
  enableAlertConfirm = '<fmt:message key='resoure.Alert.enable.Confirm'/>';
  disableAlertConfirm = '<fmt:message key='resoure.Alert.disable.Confirm'/>';
</jsu:script>
...
<table width="100%" cellpadding="5" cellspacing="0" border="0" class="ToolbarContent">
  ...
  <c:if test="${empty noButtons}">
    ...
    <td width="40%">
       <a class="buttonGreen" href="${listNewUrl}"><span style="white-space: nowrap"><fmt:message key="resource.common.button.new"/></span></a>
    </td>
    </c:if>
    <c:if test="${!newOnly}">
      <c:if test="${includeGroup}">
    <td width="40%" align="left" id="<c:out value="${widgetInstanceName}"/>GroupButtonTd"><div id="<c:out value="${widgetInstanceName}"/>GroupButtonDiv"><input id="<c:out value="${widgetInstanceName}"/>GroupButton" type="button" class="compactbuttoninactive" value="<fmt:message key='resource.common.button.group' />" disabled="disabled" /></div></td>
      </c:if>
    <%-- this is for formatting nazis (you know who you are): there is a good reason for "bad" formatting of the next line --%>
    <td width="40%" align="left" id="<c:out value="${widgetInstanceName}"/>DeleteButtonTd"><div id="<c:out value="${widgetInstanceName}"/>DeleteButtonDiv"><input id="<c:out value="${widgetInstanceName}"/>DeleteButton" type="button" class="compactbuttoninactive" value="<fmt:message key='resource.common.button.delete' />" disabled="disabled" /></div></td>
      <c:if test="${not empty ResourceSummary && empty hideAlertDefinitionActions}">
 	 	<td align="left" id="<c:out value="${widgetInstanceName}"/>EnableAlertsButtonTd"><div id="<c:out value="${widgetInstanceName}"/>EnableAlertsButtonDiv"><input id="<c:out value="${widgetInstanceName}"/>EnableAlertsButton" type="button" class="compactbuttoninactive" value="<fmt:message key='resource.common.button.EnableAllAlerts' />" disabled="disabled" /></div></td>
 	 	<td align="left" id="<c:out value="${widgetInstanceName}"/>DisableAlertsButtonTd"><div id="<c:out value="${widgetInstanceName}"/>DisableAlertsButtonDiv"><input id="<c:out value="${widgetInstanceName}"/>DisableAlertsButton" type="button" class="compactbuttoninactive" value="<fmt:message key='resource.common.button.DisableAllAlerts' />" disabled="disabled" /></div></td>
      </c:if>
    </c:if>
  </c:if>
