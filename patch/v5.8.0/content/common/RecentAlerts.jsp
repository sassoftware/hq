  		...
  		<c:when test="${recentAlertsSize > 0}">
    		<ul class="boxy">
      			<c:forEach var="alert" varStatus="status" items="${recentAlerts}">
        			<sas:evmEventDateTimeTag var="alert_ctime_title_inpage" value="${alert.ctime}"/>
        			...
        			<li class="MastheadContent">
        				<html:link href="${alertUrl}" styleClass="MastheadLink" title="${alert_ctime_title_inpage}" tabindex="-1">
        					<sas:evmServerTimeTag value="${alert.ctime}"/>
        				</html:link>
