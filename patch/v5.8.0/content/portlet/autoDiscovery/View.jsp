    ...
    <c:choose>
      ...
      <c:otherwise>
        ...
        <c:forEach items="${resources}" var="resource">
          <tr class="ListRow">
            <td class="ListCell" nowrap="nowrap"><html:radio property="platformsToProcess" value="${resource.id}" onfocus="focusOn(this);" onblur="focusOff(this);"/></td>
            <td colspan="2" class="ListCell">
              	<html:link action="/resource/platform/AutoDiscovery" onfocus="focusOn(this);" onblur="focusOff(this);">
            ...
            <td class="ListCell">
                 <c:choose>
                             <c:when test="${resource.queueStatusStr == 'new'}">
                                      <fmt:message key="dash.autodiscovery.status.new"/>
                             </c:when>
                             <c:when test="${resource.queueStatusStr == 'unchanged'}">
                                      <fmt:message key="dash.autodiscovery.status.unchanged"/>
                             </c:when>
                             <c:when test="${resource.queueStatusStr == 'modified'}">
                                      <fmt:message key="dash.autodiscovery.status.modified"/>
                             </c:when>
                             <c:when test="${resource.queueStatusStr == 'removed'}">
            ...
            <!--c:out value="${resource.queueStatusStr}"/-->
            </td>            
            ...
            <hq:skipIfAutoApproved aiserver="${server}">
              <c:if test="${server.queueStatus != 0 && !server.ignored}">
                <tr class="ListRow">
                  <td class="ListCell" nowrap="nowrap">&nbsp;&nbsp;&nbsp;<html:multibox property="serversToProcess" value="${server.id}" onfocus="focusOn(this);" onblur="focusOff(this);"/></td>
                  <td class="ListCell">
                  ...
                  <td class="ListCell">
                  <c:choose>
                             <c:when test="${server.queueStatusStr == 'new'}">
                                      <fmt:message key="dash.autodiscovery.status.new"/>
                             </c:when>
                             <c:when test="${server.queueStatusStr == 'unchanged'}">
                                      <fmt:message key="dash.autodiscovery.status.unchanged"/>
                             </c:when>
                             <c:when test="${server.queueStatusStr == 'modified'}">
                                      <fmt:message key="dash.autodiscovery.status.modified"/>
                             </c:when>
                             <c:when test="${server.queueStatusStr == 'removed'}">
                                      <fmt:message key="dash.autodiscovery.status.removed"/>
                             </c:when>
                             <c:otherwise>
                                      <fmt:message key="dash.autodiscovery.status.unknow"/>
                                       ...
                                       <!--c:out value="${server.queueStatusStr}"/-->
                  </td>
          ...
          <td colspan="5" class="ListCell" nowrap>
            <table cellpadding="0" cellspacing="0" border="0">
             <tr>
                ...
                <a class="buttonGreen" href="javascript:setImportCheckboxes(AIQueueForm); addInventory(); hyperic.form.mockLinkSubmit('buttonAction','Add to Inventory','buttonActionHidden')" onfocus="focusOn(this);" onblur="focusOff(this);"><span style="white-space: nowrap"><fmt:message key="common.label.AddtoInventory"/></span></a>
                <a class="buttonGreen" href="javascript:skipResources(); hyperic.form.mockLinkSubmit('buttonAction','Skip Checked Resources','buttonActionHidden');" onfocus="focusOn(this);" onblur="focusOff(this);"><span style="white-space: nowrap"><fmt:message key="common.label.SkipResources"/></span></a>
                </td>
            </tr>
           </table>
            </td>
