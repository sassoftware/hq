...
var searchstr = '[ Group Search ]';
var searchResstr = '[ Resources ]';
var gropManaStr = 'Group Manager';
var alertTitlestr = 'Alert Center';
var alertFixHintstr = 'Resolution for Fix for Selected Alerts:';
var alertFixPrevStr = 'Fix all previous alerts';
var alertBtnFixed = 'Fixed';
var alertactHintstr = 'Reason for Acknowledgement for Selected Alerts:';
var alertPauseEscStr = 'Pause escalation for:';
var alertBtnAct = 'Acknowledge';
var alertBtnProcess = 'Please wait. Processing your request...';
...
var playSlideShow='play slideshow';
var pauseSlideShow='pause slideshow';

var groupAlertFailure='Failure';
var groupAlertWarning='Warning';
var groupAlertOK='OK';
var groupAlertNoData='No Data';
...
function activateHeaderTab(dojo){
    ...
    if ( l.indexOf("Dash")!=-1 ||
    	...
    	dojo.addClass("adminTab", "activeTab");
    }else if( l.indexOf("EnvironmentManagerInvoker") != -1){
		dojo.addClass("evTab", "activeTab");
    } else if ( l.indexOf("/sas/") != -1 ) {
    	dojo.addClass("evTab", "active");
    }
...
hyperic.dashboard.chartWidget = function(args) {
    ...
    that.playCharts = function() {
        ...
        if(that.cycleId === null) {
            ...
            that.play_btn.alt = pauseSlideShow;
            that.play_btn.title = pauseSlideShow;
        }
    };
    ...
    that.pauseCharts = function() {
        if(that.cycleId !== null) {
            ...
            that.play_btn.alt = playSlideShow;
            that.play_btn.title= playSlideShow;
        }
    };
...
hyperic.dashboard.summaryWidget = function(args) {
    ...
    that.paintAlertGroups = function()
    {
        ...
        var status = {
            'red'    : groupAlertFailure,
            'green'  : groupAlertOK,
            'yellow' : groupAlertWarning,
            'gray'   : groupAlertNoData
        };
        ...
        for(var i = 0; i < groups.length; i++)
        {
            ...
            row.childNodes[1].innerHTML = '<img src="/images/4.0/icons/'+data[0]+'.gif" title="'+ status[data[0]] +'">';
            row.childNodes[2].innerHTML = '<a href="/alerts/Alerts.do?mode=list&eid=5:' + groups[i] + '"><img src="/images/4.0/icons/'+data[1]+'.gif" title="'+ status[data[1]]+'" border="0"></a>';
...
hyperic.group_manager = function(args) {
	...
	that.prepareAddResourcesToGroups = function(eidArray) {
		if(hqDojo.byId("AddToExistingGroup_CheckAllBox")){
			hqDojo.byId("AddToExistingGroup_CheckAllBox").checked=false;
		}
		...
		hqDojo.xhrPost( {
            ...
            load: function(data) {
				if (data && data.error) {
				    ...
				    successText = alertAddExistingSucessText;
            ...
            error: function(data) {
	    		...
	    		errorText = alertAddExistingErrorText ;
...
hyperic.indicator_charts_manager = function(props, charts) {
	...
	that.drawChart = function(chartArgs) {
		...
		var liInnerHtml =
		    ...
		    '    <tr>' +
		    '      <td class="BoldText">' +
		    '        <img src="/images/icon_chart.gif" title="' + that.labels.chartTitle + '" alt="' + that.labels.chartTitle + '"/>' +
		    '        <a href="' + that.getChartLink(chartArgs) + '" target="_top">' + chartArgs.metricLabel + '</a>' +
		    '      </td>' +
		    '      <td colspan="3" align="right">' +
		    '        <a href="javascript:' + that.name + '.moveMetricUp(\'' + chartArgs.metricId + '\')"><img src="/images/dash_icon_up.gif" border="0" title="' + that.labels.moveupTitle + '" alt="' + that.labels.moveupTitle + '"/></a>' +
		    '        <a href="javascript:' + that.name + '.moveMetricDown(\'' + chartArgs.metricId + '\')"><img src="/images/dash_icon_down.gif" border="0" title="' + that.labels.movedownTitle + '" alt="' + that.labels.movedownTitle + '"/></a>' +
		    '        <a href="javascript:' + that.name + '.removeMetric(\'' + chartArgs.metricId + '\')"><img src="/images/dash-icon_delete.gif" border="0" title="' + that.labels.removeTitle + '" alt="' + that.labels.removeTitle + '"/></a>' +
		    '      </td></tr>' +
		    '    <tr>' +
...
hyperic.alert_center = function(title_name) {
	...
	that.initDialog = function() {
    	...
    	if (!that.dialogs.AckAlert) {
	    	...
	    	ackDiv.innerHTML =
				...
				'<tr><td colspan="2">' +
      	        '	<span class="BoldText">' + alertactHintstr + '</span><br/>' +
      	        ...
      	        '		' + alertPauseEscStr + '&nbsp;<select id="AlertCenterPauseSelect"></select></span></td>' +
      	        '	<td align="right">' +
      	    	'		<table border="0" cellspacing="0" cellpadding="0"><tr>'+
      	        '			<td class="buttonLeft"></td>' +
      	    	'			<td class="buttonRight" valign="middle" nowrap="nowrap" style="padding-top: 6px; padding-bottom: 6px;">' +
      	    	'				<span id="button"><a style="color:#406469;" href="javascript:MyAlertCenter.acknowledgeAlerts();">' + alertBtnAct + '</a></span>' +
      	    	'			</td></tr></table>' +
      	    	'	</td></tr>' +
      	        ...
      	        '			<td class="buttonLeft"></td>' +
      	    	'			<td class="buttonRight" valign="middle" nowrap="nowrap" style="padding-top: 6px; padding-bottom: 6px;">' +
      	    	'   			<span class="InactiveText">' + alertBtnAct + '</span>' +
      	    	'			</td></tr></table>' +
      	    	'	</td></tr>' +
      	    	'</table>' +
      	    	'</div>';
		...
		if (!that.dialogs.FixAlert) {
			...
			fixedDiv.innerHTML =
				...
				'<table cellspacing="0" cellpadding="0">' +
				'<tr><td colspan="2">' +
      	        '	<span class="BoldText">' + alertFixHintstr + '</span><br/>' +
      	        '	<textarea id="FixedNoteTextArea" style="width:400px; height:100px"></textarea>' +
      	        '</td></tr>' +
      	        '<tr id="AlertCenterFixedButtonActive">' +
      	        '	<td><input type="checkbox" id="FixAllCheckbox" value="true" />&nbsp;' + alertFixPrevStr + '</td>' +
      	    	'	<td align="right">' +
      	    	'		<table border="0" cellspacing="0" cellpadding="0"><tr>'+
      	    	'			<td class="buttonLeft"></td>' +
      	    	'			<td class="buttonRight" valign="middle" nowrap="nowrap" style="padding-top: 6px; padding-bottom: 6px;">' +
      	    	'				<span id="button"><a style="color:#406469;" href="javascript:MyAlertCenter.fixAlerts();">' + alertBtnFixed + '</a></span>' +
      	    	'			</td></tr></table>' +
      	    	'	</td></tr>' +
      	        ...
      	        '			<td class="buttonLeft"></td>' +
      	    	'			<td class="buttonRight" valign="middle" nowrap="nowrap" style="padding-top: 6px; padding-bottom: 6px;">' +
      	    	'				<span class="InactiveText">' + alertBtnFixed + '</span>' +
      	    	'			</td></tr></table>' +
      	    	'	</td></tr>' +
      	    	'</table>' +
      	    	'</div>';
	...
	that.processButtonAction = function(myButton) {
		if (myButton.id.indexOf("FixButton")>-1) {
			myButton.form.buttonAction.value = "FIXED";

			that.initData(that.dialogs.FixAlert, myButton.form);
			that.showDialog(that.dialogs.FixAlert);
		} else if (myButton.id.indexOf("AckButton")>-1) {
			myButton.form.buttonAction.value = "ACKNOWLEDGE";
...
hyperic.maintenance_schedule = function(args) {
    ...
    that.init = function() {
		...
		that.buttons.cancel_schedule_btn = new hqDijit.form.Button({
			label: hyperic.data.maintenance_schedule.label.cancel,
			name: "cancel_schedule_btn",
			id: "cancel_schedule_btn",
			type: 'button'
		}, "cancel_schedule_btn");
        hqDojo.connect(that.buttons.cancel_schedule_btn, 'onClick', function(){hqDijit.byId("maintenance_schedule_dialog_" + that.entity_id).onCancel();});
    ...
    that.redraw = function(show, scheduleStatus, confirmationStatus)
    {
		...
		hqDojo.byId('maintenance_from_time_timezone').innerHTML = hyperic.data.maintenance_schedule.label.time;
		hqDojo.byId('maintenance_to_time_timezone').innerHTML = hyperic.data.maintenance_schedule.label.time;
        ...
        hqDojo.addClass(hqDojo.byId("schedule_btn_label"),'BlackFontColor');
		hqDojo.addClass(hqDojo.byId("cancel_schedule_btn_label"),'BlackFontColor');
		hqDojo.addClass(hqDojo.byId("from_date"),'BlackFontColor');
		hqDojo.addClass(hqDojo.byId("from_time"),'BlackFontColor');
		hqDojo.addClass(hqDojo.byId("to_date"),'BlackFontColor');
		hqDojo.addClass(hqDojo.byId("to_time"),'BlackFontColor');
		hqDojo.addClass(hqDojo.byId('existing_downtime_' + that.entity_id),'BlackFontColor');		
...
hyperic.clone_resource_dialog = function(args) {
    ...
    var clonebtnName= hqDojo.byId('clone_btn').name;
    ...
    var cancelbtnName= hqDojo.byId('clone_cancel_btn').name;
        ...
        hqDojo.connect(that.searchbox,'onfocus', function(e) {if(e.target.value == searchResstr) { e.target.value = ''; }});
        hqDojo.connect(that.searchbox,'onblur', function(e) {if(e.target.value == '') { e.target.value = searchResstr; }});
...
Date.prototype.formatDate = function(format)
{
    ...
    if (format.indexOf("t") > -1)
    {
       if (hours > 11)
       {
           format = format.replace("t",curPM);
       }
       else
       {
           format = format.replace("t",curAM);
       }
    }
...
hyperic.MetricsUpdater = function(eid,ctype,messages) {
            ...
            hqDojo.xhrGet( {
                ...
                load: function(data, ioArgs) {
                    ...
                    if (hqDojo.byId('UpdatedTime') !== null) {
                        hqDojo.byId('UpdatedTime').innerHTML = messages.LastUpdated + " " + data.updatedtime;
                    }
                },
                error: function(data){
    ...
    that.substitute = function( attribute, metricValues) {
        ...
        if (lastSpan !== null&&lastSpan!=undefined) {
            var html;
            if (attribute == "avail") {
                var img;
...
hyperic.MetricChart = function(formObj) {
	...
	that.generateChartParameters = function() {
		...
		for (var x = 0; x < inputEls.length; x++) {
			...
			if ((el.type == 'checkbox' && !el.checked)||el.type == 'button') continue;
...
{
    return '<a title="' + str.replace("\"","&#34;") + '" href="'+ url +'" onfocus="focusOn(this);" onblur="focusOff(this);">' + fitStringToSize(str,width) + '<\/a>';
}
