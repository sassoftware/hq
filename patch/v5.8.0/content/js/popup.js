...
var overlay = {
    ...
    showTopN: function (index) {
        requestTopN(this.timesREQ[index]);
    },
	...
	showTimePopupTopMetricChartRelated: function (index, e, anchorTop) {
        this.hideTimePopup();
        var anchor = hqDojo.byId('timePopup_' + index);
        var left = this.findPosX(anchor) - 35;
        var top = this.findPosY(anchor) + 10;
        ...
        this.curPopup.innerHTML = this.times[index];
        this.curPopup.style.left = left + 'px';
		var topRelated = this.findPosY(anchorTop)- 38;
        if (e)
            this.curPopup.style.top = topRelated + 'px';
        else
            this.curPopup.style.top = top + 'px';
...
var menuLayers = {
  timer: null,
  activeMenuID: null,
  offX: 6,   // horizontal offset 
  offY: 5,   // vertical offset  
  
  show: function(id, e) {
    var mnu = hqDojo.byId(id);
    if (!mnu) return;
	if(this.activeMenuID != null) hqDojo.style(menuLayers.activeMenuID, "visibility", "hidden");
