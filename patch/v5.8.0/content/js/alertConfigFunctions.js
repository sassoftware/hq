...
*/
  if (masterSelValue == '') {
    ...
    for (i=0; i<baselines[masterSelValue].length-1; i++) {
	  if(baselines[masterSelValue][i].label.indexOf('Baseline Value')>-1)
    	{
		    var vnum=baselines[masterSelValue][i].label.indexOf('Baseline Value');
			var vlen=baselines[masterSelValue][i].label.length;
    		baselines[masterSelValue][i].label= baselines[masterSelValue][i].label.substring(0,vnum) + baselinestr + baselines[masterSelValue][i].label.substring(vnum +14,vlen) ;
    	}
    	else if(baselines[masterSelValue][i].label.indexOf('Min Value')>-1)
    	{
		    var vnum=baselines[masterSelValue][i].label.indexOf('Min Value');
			var vlen=baselines[masterSelValue][i].label.length;
			baselines[masterSelValue][i].label= baselines[masterSelValue][i].label.substring(0,vnum) + minvaluestr + baselines[masterSelValue][i].label.substring(vnum +9,vlen) ;

    	}
    	else if(baselines[masterSelValue][i].label.indexOf('Max Value')>-1)
    	{
		    var vnum=baselines[masterSelValue][i].label.indexOf('Max Value');
			var vlen=baselines[masterSelValue][i].label.length;
			baselines[masterSelValue][i].label= baselines[masterSelValue][i].label.substring(0,vnum) + maxvaluestr + baselines[masterSelValue][i].label.substring(vnum +9,vlen) ;
    	}
      if (isIE) {
