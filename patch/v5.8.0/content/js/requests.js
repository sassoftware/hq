
{
    ...
    function showProblemResponse(response, args) {
        ...
        hqDojo.byId('modifiedProblemTime').innerHTML = updatedStr + refreshTime();
    ...
    var recentalerts_yes = "Yes";
    var recentalerts_no = "No";
    var recentalerts_acknowledge = "Acknowledge";
    
    function setRecentAlertsResources(stryes, strno, strack) {
    	recentalerts_yes = stryes;
    	recentalerts_no = strno;
    	recentalerts_acknowledge = strack;
    }
    ...
    function showRecentAlerts(response, args) {
        ...
        if (aList.length != 0) {
            ...
            for (i = 0; i < aList.length; i++) {
                ...
                if (aList[i].fixed) {
                    td5.appendChild(document.createTextNode(recentalerts_yes));
                } else {
                    td5.appendChild(document.createTextNode(recentalerts_no));
                }
    ...
    function showAvailSummary(response, args) {
        ...
        if (availList.length < 1) {
            ...
            for (var i = 0; i < availList.length; i++) {
                ...
                var downgraphic = '<span style="padding-right:5px;padding-left:5px;"><img src=/images/icon_available_red.gif></span>';
            ...
            if (token != null) {
                ...
                hqDojo.byId('availTime' + token).innerHTML = updatedStr + refreshTime();
            } else {
                td4.setAttribute('id', 'availTime');
                hqDojo.byId('availTime').innerHTML = updatedStr + refreshTime();
            }
    ...
    function showMetricsResponse(response, args) {
        ...
        if (metricTable && metricValues.values) {
            ...
            if (token != null) {
                ...
                hqDojo.byId('metricTime' + token).innerHTML = updatedStr + refreshTime();
            } else {

                td3.setAttribute('id', 'metricTime');
                hqDojo.byId('metricTime').innerHTML = updatedStr + refreshTime();
            }
        } else {
    ...
    function showFavoriteResponse(response, args) {
        ...
        hqDojo.byId('modifiedFavoriteTime').innerHTML = updatedStr + refreshTime();
        var maxResourceNameSize;
