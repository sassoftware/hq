...
<display:table items="${users}" var="user" action="${sAction}" width="100%" cellpadding="0" cellspacing="0">
  ...
  <display:column width="25%" property="name" title="admin.role.users.UsernameTH" sort="true" sortAttr="3" defaultSort="true"/>
  <display:column width="25%" property="firstName" title="admin.role.users.FirstNameTH"/>
  <display:column width="25%" property="emailAddress" title="admin.role.users.EmailTH"/>
  <display:column width="25%" property="department" title="admin.role.users.DepartmentTH"/>
</display:table>

<jsu:script>
	var containerTable = hqDojo.byId("migContainer");
	if(containerTable!=null&&containerTable!='undefined'){
		var tds = hqDojo.query(".ListCell", containerTable);
		for(var i=0;i<tds.length;i++){
			var td = tds[i];			
			var img = hqDojo.query(">input[type=image]",td)[0];			
			img.title="<fmt:message key="resource.common.inventory.changeresourceowner.selectuser" />";			
			img.alt="<fmt:message key="resource.common.inventory.changeresourceowner.selectuser" />";
		}
	}
</jsu:script>
