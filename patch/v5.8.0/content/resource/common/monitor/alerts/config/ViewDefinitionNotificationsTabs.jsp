...
<c:forEach var="tab" items="${tabList}">
  ...
  <c:choose>
    <c:when test="${mode == tab.link}">
      <td><span><fmt:message key="monitoring.events.MiniTabs.${tab.value}"/></span></td>
    </c:when>
    ...
    <c:otherwise>
      <td>
        <a href="${tabUrl}"><span><fmt:message key="monitoring.events.MiniTabs.${tab.value}"/></span></a>
      </td>
    </c:otherwise>
  </c:choose>
</c:forEach>
