...
<jsu:script>
   ...
   <c:forEach var="timeTick" items="${timeIntervals}">
      overlay.times.push('<sas:evmTimelineTag value="${timeTick.time}"/>');
      overlay.timesREQ.push('<hq:dateFormatter value="${timeTick.time}"/>');
   </c:forEach>
...
<table cellpadding="0" cellspacing="0" border="0" width="100%">
 ...
 <tr>
     <td width="8" valign="top"><div id="timetop"></div><html:img page="/images/timeline_ll.gif" height="10"/></td>
     <c:forEach var="timeTick" items="${timeIntervals}" varStatus="status">
         ...
         <c:if test="${isPlatform}">
     <td width="10" align="left" valign="top"><html:img page="/images/timeline_lr.gif" height="10"/></td>
         </c:if>
         <c:if test="${not isPlatform}">
     <td width="100%" valign="top"><html:img page="/images/timeline_lr.gif" height="10"/></td>
         </c:if>
     <c:if test="${isPlatform}">
         <td class="BoldText" style="padding-left: 4px;" nowrap>
 ...
 <tr>
   ...
   <td colspan="<c:out value="${count / 2}"/>" valign="top">
     <sas:evmTimelineTag value="${timeIntervals[0].time}"/>
   ...
   <td colspan="<c:out value="${count / 2}"/>" align="right" valign="top">
     <sas:evmTimelineTag value="${timeIntervals[count - 1].time}"/>
   </td>
   <td></td>
 </tr>
 <tr>
