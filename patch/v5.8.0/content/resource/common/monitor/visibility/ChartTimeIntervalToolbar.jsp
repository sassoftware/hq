...
<table width="100%" cellpadding="0" cellspacing="0" border="0" id="advancedTbl">
  ...
  <tr>
    <td class="BlockContent" width="100%" align="right">
      <html:image property="${prevProperty}" page="/images/tbb_pageleft.gif" border="0" altKey="button.prev"/>
    </td>
    <td class="BlockContent" nowrap>
      <sas:evmTimelineTag value="${begin}"/>
      &nbsp;<fmt:message key="resource.common.monitor.visibility.chart.to"/>&nbsp;
      <sas:evmTimelineTag value="${end}"/>
    </td>
    <td class="BlockContent">
      <c:choose>
      <c:when test="${rangeNow}">
      <!-- <html:img page="/images/tbb_pageright_gray.gif" border="0" altKey="button.next"/> -->
      </c:when>
      <c:otherwise>
      <html:image property="${nextProperty}" page="/images/tbb_pageright.gif" border="0" altKey="button.next"/>
      </c:otherwise>
      </c:choose>
    </td>
  </tr>
