...
<jsu:script>
	...
	function showViewEscResponse(response, args) {
		...
		if (tmp.error) {
    	    ...
    	    if(tmp.error=="An escalation with this name already exists."){
	          escmsg.innerHTML = '<fmt:message key="escalation.name.already.exist" />';
	        }
	        ...
	        if(id==100){
	        if(escName=='Default Escalation'){
	         escName='<fmt:message key="alert.config.escalation.defaultEscalationName"/>';
	        }
	        if(description='This is an Escalation Scheme created by HQ that performs no actions'){
	         description='<fmt:message key="alert.config.escalation.defaultEscalationDescription"/>';
	        }
	        }
	        ...
	        for (i = 0; i < actions.length; i++) {
	            ...
	            td6.innerHTML = '<div><fmt:message key="common.label.escalation.actiondetail" /></div>';
		        ...
		        <c:if test="${useroperations['modifyEscalation']}">
		            ...
		            td8.innerHTML = '<a href="#" onclick="removeRow(this);removeAction(' + actionId + ');"><fmt:message key="resource.common.button.delete" /></a>';
		        </c:if>
	            ...
	            waitDiv.innerHTML = '<fmt:message key="common.label.escalation.wait.time"/> ' + actionWaitTime + '<br>';
                ...
                for (var d = 0; d < actionClass.length; d++) {
                    if (actionClass[d] == "SyslogAction") {
                        usersTextDiv.innerHTML = '<table cellpadding="0" cellspacing="0" border="0"><tr><td rowSpan="3" vAlign="top" style="padding-right:3px;"><fmt:message key="admin.config.escalation.log.to.sys.log" /></td><td style="padding:0px 2px 2px 2px;"><fmt:message key="common.label.escalation.action.syslog.meta" /> ' + configMeta + '</td></tr><tr><td style="padding:2px;"><fmt:message key="common.label.escalation.action.syslog.product" /> ' + configProduct + '</td></tr><tr><td style="padding:2px 2px 2px 2px;"><fmt:message key="common.label.escalation.action.syslog.version" /> ' + configVersion + '</td></tr></table>'
                    } else if (actionClass[d] == "NoOpAction") {
                        usersTextDiv.innerHTML = '<fmt:message key="admin.config.escalation.suppress.duplicate.alerts.for" />: ' + actionWaitTime;
                        waitDiv.innerHTML = "&nbsp;";
                         ...
                         var snmpInnerHTML = '<table cellpadding="0" cellspacing="0" border="0">'
                                ...
                                + '<td><fmt:message key="common.label.escalation.param.values"/> {snmp_trap.gsp}</td></tr>';
                            ...
                            for (var s = 0; s < configSnmpVarBinds.length; s++) {       
                                snmpInnerHTML += '<tr><td style="padding-right:5px;"><fmt:message key="alert.config.escalation.action.snmp.oid"/>: '
                                        ...
                                        + '<td><fmt:message key="common.label.escalation.param.values"/> ' + configSnmpVarBinds[s].value + '</td></tr>';
                            }
		...
		//alert(roleNames);
	            ...
	            hqDojo.byId('pauseTimeText').innerHTML = '<fmt:message key="admin.config.escalation.allow.user.pause"/>: ' + allowPause + "<br>";
      		}
	...
	function saveEscalation() {
		var selectedName = hqDojo.query('.selectedHighlight')[0];	
	    ...
	    if (escName.match(/['"]/) || escDesc.match(/['"]/)) {
	   	...
	   	hqDojo.xhrPost({
	   		...
	   		load: function(response, args){showViewEscResponse(response, args);hqDojo.query(selectedName,'b')[0].innerHTML=escName.escapeHTML()},
	   		error: reportError
	   	});
	    hqDojo.byId('escPropertiesTable').style.display = '';
	    hqDojo.byId('editPropertiesTable').style.display = 'none';
	...
	function updateEscView(originalRequest) {
	    ...
	    hqDojo.byId('escMsg').innerHTML = "<fmt:message key='common.label.escalation.action.added' />";
	    cancelAddEscalation();
	...
	function addRow() {
	    ...
	    td6.setAttribute('colSpan', '3');
	    td6.setAttribute((document.all ? 'className' : 'class'), "BlockTitle");
	    td6.innerHTML = '<div><fmt:message key='common.label.escalation.actiontile'/></div>';
	    ...
	    td5.innerHTML = '<table cellpadding="2" cellspacing="0" border="0" width="100%">'
	 		+ '<tbody><tr><td class=BlockTitle colSpan=3><fmt:message key='common.label.escalation.actiondetail'/></td></tr>'
	 	 	+ '<tr><td id="actionName" vAlign="top" width="50%"><fmt:message key='common.label.escalation.action'/>  <fmt:message key='common.label.escalation.action.select'/></td></tr>'
			...
			+ '<tr><td id=TrapOIDText style="display:none"></td></tr>'
	 	 	...
	 	 	+ '<tr><td id=OIDValue style="display:none"><fmt:message key="common.label.escalation.param.values" /> {snmp_trap.gsp}</td></tr>'
	    ...
	    addOption(select2, 'Email', '<fmt:message key="alert.config.escalation.action.Email"/>');
	    addOption(select2, 'SMS', '<fmt:message key="alert.config.escalation.action.SMS"/>');
	    addOption(select2, 'Syslog', '<fmt:message key="alert.config.escalation.action.Syslog"/>');
		<c:if test="${snmpEnabled}">
		    addOption(select2, 'SNMP', '<fmt:message key="alert.config.escalation.action.snmp.notification"/>');
		</c:if>
	    addOption(select2, 'NoOp', '<fmt:message key="alert.config.escalation.action.NoOp"/>');
	    escTr2.appendChild(td3);
	    ...
	    sysDiv.innerHTML = "<fmt:message key="common.label.escalation.action.syslog.meta"/><br> <input type=text name=meta id=metainput" + " size=30 onblur=copyMeta(this);><br>" + "<fmt:message key="common.label.escalation.action.syslog.product"/><br> <input type=text name=product id=productinput" + " size=30 onblur=copyProduct(this);><br>" + "<fmt:message key="common.label.escalation.action.syslog.version"/><br> <input type=text name=version id=versioninput" + " size=30 onblur=copyVersion(this);><br>";
	    ...
	    snmpDiv.innerHTML = '<fmt:message key="resource.autodiscovery.server.IPAddressTH"/>: <fmt:message key="inform.config.escalation.scheme.IPAddress"/><br>'
	 	 	+ '<input type=text name=snmpIP id=snmpIPinput size=30 onblur=copysnmpIP(this);><br>'
			...
			+ '<input type=text name=snmpTrapOID id=snmpTrapOIDinput size=30 disabled onblur=copysnmpTrapOID(this);><br>'
	 	 	...
	 	 	+ '<input type=text name=snmpOID id=snmpOIDinput style="width:250px" onblur=copysnmpOID(this);><br>'
	  	 	+ '<fmt:message key="common.label.escalation.param.values" /> {snmp_trap.gsp}<br>'
	...
	function copyOthersEmail(el) {
	    ...
	    othersDisplay.innerHTML = '<fmt:message key="common.label.escalation.action.sms.notifyemail"/>' + el.value;
	}
	
	function copyMeta(el) {
	    var metaDisplay = hqDojo.byId('metaText');
	    metaDisplay.style.display = "";
	    metaDisplay.innerHTML = '<fmt:message key="common.label.escalation.action.syslog.meta"/> ' + el.value;
	}
	
	function copyProduct(el) {
	    var productDisplay = hqDojo.byId('productText');
	    productDisplay.style.display = "";
	    productDisplay.innerHTML = '<fmt:message key="common.label.escalation.action.syslog.product"/> ' + el.value;
	}
	
	function copyVersion(el) {
	    var versionDisplay = hqDojo.byId('versionText');
	    versionDisplay.style.display = "";
	    versionDisplay.innerHTML = '<fmt:message key="common.label.escalation.action.syslog.version"/> ' + el.value;
	}
		...
		for (var s = 0; s < vbOids.length; s++) {
			if (vbOids[s].value.length > 0 && vbValues[s].value.length > 0) {
				...
				vbsInnerHTML += '<fmt:message key="alert.config.escalation.action.snmp.oid"/>: ' 
	    						+ vbOids[s].value + '<br/>'
					   			+ '<fmt:message key="common.label.escalation.param.values" /> ' + vbValues[s].value + '<br/>';
			}
		}
    ...
    function onchange_handler(el) {
        ...
        if (index == "NoOp") {
            ...
            writeAction.innerHTML = '<fmt:message key="common.label.escalation.action"/> ' + index;
            if(index=='Syslog'){
             writeAction.innerHTML = '<fmt:message key="common.label.escalation.action"/> ' + '<fmt:message key="common.label.escalation.action.syslog"/>';
            }
            if(index=='Select'){
             writeAction.innerHTML = '<fmt:message key="common.label.escalation.action"/> ' + '<fmt:message key="common.label.escalation.action.select"/>';
            }
            if(index=='Email'){
             writeAction.innerHTML = '<fmt:message key="common.label.escalation.action"/> ' + '<fmt:message key="common.label.escalation.action.email"/>';
            }
	...
	function onchange_time(el) {
	    ...
	    var index = el.options[el.selectedIndex].value;
	    writeTime.innerHTML = '<fmt:message key='common.label.escalation.action.wait'/> ' + formatWaitTime(el, null, '<fmt:message key="alert.config.props.CB.Enable.TimeUnit.2"/>',  '<fmt:message key="alert.config.props.CB.Enable.TimeUnit.1"/>');
	}
	...
	function showResponseRemoved() {
	    hqDojo.byId('example').style.display = '';
	    hqDojo.byId('escMsg').innerHTML = '<fmt:message key="common.label.escalation.action.removed" />';
	...
	function configureOthers(el) {
	    ...
	    {windowParameters: {className:'dialog', width:305, height:200,
	        ...
	        okLabel: "<fmt:message key="button.ok"/>", cancelLabel: "<fmt:message key="button.cancel"/>",
	        ok:function(win) {
	            // Do something
	...
	function configureUsers(el) {
	    ...
	    {windowParameters: {className:'dialog', width:305, height:200,
	        ...
	        ok:function(win) {
	            ...
	            writeListUsers.appendChild(document.createTextNode('<fmt:message key="common.label.escalation.action.notify"/> '));
	            var hasEle = false;
	            for (i = 0; i < usersInputList.length; i++) {
	                ...
	                if (updatedInputList[i].checked) {
	                	...
	                	usersInputList[i].setAttribute("checked", "");
	                	usersInputList[i].removeAttribute("checked");
	                }
	...
	function isIPInvalid(iptext){
		var RegExp = /^(2[5][0-5]|2[0-4]\d|1\d{2}|\d{1,2})\.(25[0-5]|2[0-4]\d|1\d{2}|\d{1,2})\.(25[0-5]|2[0-4]\d|1\d{2}|\d{1,2})\.(25[0-5]|2[0-4]\d|1\d{2}|\d{1,2})\/(([0-9]|[1-9]\d|[1-9]\d{2}|[1-9]\d{3}|[1-5]\d{4}|6[0-4]\d{3}|65[0-4]\d{2}|655[0-2]\d|6553[0-5]))$/;
		if(RegExp.test(iptext)) {
    ...
    function isOIDInvalid(OID){ 
    	var RegExp =  /^(\d{1})\.(\d{1})\.(\d{1})\.(\d{1})$/;
		if(RegExp.test(OID)) {
			console.log("Trap OID is valid");
			return false;
		}else{
			console.log("Trap OID is invalid");
			return true;
		}
    }
    ...
    function checkOID() {
        ...
        if (OIDText == '' || isOIDInvalid(OIDText)) {
            showErrorDisplay();
            hqDojo.byId('escMsg').innerHTML = '<fmt:message key="error.Error.Tab"/> ' + '<fmt:message key="admin.config.message.IncorrectSNMPTrapOIDFormat"/>';
    ...
    function selUserEscNull() {
        ...
        if ((selUserEsc == undefined || selUserEsc == 'Select' || ((selUserEsc == 'Users'||selUserEsc == 'Roles') && (selUserEscNull_selUserOrRoleCounter == 0)) ) && (selActionTypeEsc != "Syslog" && selActionTypeEsc != "SNMP" && selActionTypeEsc != "NoOp")) {
            showErrorDisplay();
            hqDojo.byId('escMsg').innerHTML = '<fmt:message key="error.Error.Tab"/> ' + '<fmt:message key="alert.config.error.noUserSelected"/>';
            return false;
        } else {
    ...
    <td class="BlockContent">
        <table width="100%" cellpadding="4" cellspacing="0">
            <tr>
                ...
                <td width="80%"><input type="text" size="30" name="name" maxlength="30"
                                       id="escName"/></td>
            </tr>
            <tr>
                <td class="BlockLabel" valign="top">
...
<div id="usersList" style="display:none;">
    <div class="ListHeader"><fmt:message key="common.label.escalation.action.sms.email.users"/></div>
    <ul class="boxy">
        <c:forEach var="user" items="${AvailableUsers}" varStatus="status">
...
<c:if test="${not empty AvailableRoles}">
    <div id="rolesList" style="display: none;">
        <div class="ListHeader"><fmt:message key="common.label.escalation.action.sms.email.roles"/></div>
        <ul class="boxy">
            <c:forEach var="role" items="${AvailableRoles}" varStatus="status">
...
<form name="viewEscalation" id="viewEscalation">
	...
	<div>
		<div class="BlockTitle" style="position:relative;">
			<span id="step2create" style="display:none;"><fmt:message key="common.label.escalation.Step2Create"/></span>
