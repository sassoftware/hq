                        ...
                        <c:if test="${not empty param.escId && useroperations['createEscalation']}">
                        	<td class="BlockTitle" id="createButton" style="text-align: right;">
                        		<html:link action="/admin/config/Config.do">
                        			...
                        			<span><fmt:message key="resource.common.button.new" /></span>
                        		</html:link>
                        	</td>
                        </c:if>
                </tr>
...
<fmt:message key="resource.common.button.delete" />
</c:if>
</span>
<jsu:script>
  		...
  		for (var i = 0; i < schemes.length; i++) {
    		...
    		if (schemes[i].id == '<c:out value="${param.escId}"/>') {
    		    if(schemes[i].id==100 && schemes[i].name=='Default Escalation'){
    		      td2.innerHTML = '<html:img page="/images/icon_right_arrow.gif" border="0" width="10" height="10" style="padding-right:5px;"/>' + '<b>' + '<fmt:message key="alert.config.escalation.defaultEscalationName"/>' + '</b>';
    		     }
    		    else{
    		    ...
    		    if(schemes[i].id==100 && schemes[i].name=='Default Escalation'){
    		      td2.innerHTML = '<html:img page="/images/icon_right_arrow.gif" border="0" width="10" height="10" style="display:none;padding-right:5px;"/>' + '<a href="' + unescape(editEscUrl).replace("{escId}", schemes[i].id) + '" onclick="changeHighlight(this);">' + '<fmt:message key="alert.config.escalation.defaultEscalationName"/>' + '</a>';
    		     }
    		    else{
    		...
    		if (schemes.length > 1) {
    		<c:choose>
    		<c:when test="${useroperations['removeEscalation']}">
      			td3.innerHTML = '<a href="javascript:confirmDeleteEscalation(\'' + unescape(removeEscUrl).replace("{escId}", schemes[i].id) + '\')" >' + hqDojo.byId('deleteBtn').innerHTML + '</a>';
      		</c:when>
      		<c:otherwise>
     			td3.innerHTML="&nbsp;";
      		</c:otherwise>
	...
	function confirmDeleteEscalation(hrefLink){
		if(confirm('<fmt:message key="admin.config.listescalationschemes.delete.confirmmassage"/>')	){
