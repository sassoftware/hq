...
import org.hyperic.hq.ui.util.trans.AppKeyValueMap
    ...
    extends BaseController
{
    ...
    private getAgentSchema() {
        def res = [
            ...
            columns: [
                ...
                [field: [getValue: {localeBundle.serverAddress},
                 description:'serverAddress', sortable:false],
                 width: '15%',
                 label: {it.serverHtml}],
                [field: [getValue: {localeBundle.agentPort},
                 description:'agentPort', sortable:false],
                 width: '5%',
                 label: {it.agent.port}],
                [field: [getValue: {localeBundle.agentversion},
                 description:'agentversion', sortable:false],
                 width: '5%',
                ...
                [field: [getValue: {localeBundle.agentCreateTime},
                 description:'agentCreateTime', sortable:false],
                 width: '10%',
    ...
    private getCacheSchema() {
        def regionCol = new CacheColumn('region', localeBundle['region'], true)
        def sizeCol   = new CacheColumn('size', localeBundle['size'], true)
        def hitsCol   = new CacheColumn('hits', localeBundle['hits'], true)
        def missCol   = new CacheColumn('misses', localeBundle['misses'],true)
        def limitCol  = new CacheColumn('limit', localeBundle['limit'],true)
    ...
    private getInventorySchema() {
        def resourceTypeCol = new CacheColumn('resourceType', localeBundle['resourceType'], true)
        def totalCol   = new CacheColumn('total', localeBundle['total'], true)
    ...
    def getDiag(params) {
		LocaleContextHolder.setLocale(LocaleSetId.getLocale())
    ...
    private formatBytes(b) {
        if (b == -1)
            return AppKeyValueMap.valueFor('resource.hub.legend.unknown', locale)
    ...
    def getSystemStats(params) {
        ...
        def loadAvgFmt = new PrintfFormat(LocaleSetId.getLocale(), '%.2f')
        def dateFormat = DateFormat.getDateTimeInstance(DateFormat.MEDIUM, DateFormat.LONG, LocaleSetId.getLocale())
        
        def cpu      = s.cpuPerc
        def sysMem   = s.mem
        def sysSwap  = s.swap
        def pid      = s.pid
        def procFds  = AppKeyValueMap.valueFor('resource.hub.legend.unknown', LocaleSetId.getLocale())
        def procMem  = s.getProcMem(pid)
        def procCpu  = s.getProcCpu(pid)
        def procTime = s.getProcTime(pid)
        def NA       = AppKeyValueMap.valueFor('common.value.notavail', LocaleSetId.getLocale())
        def loadAvg1 = NA
        def loadAvg5 = NA
        def loadAvg15 = NA
    ...
    private getDatabaseQueries() {
        def queries = [ 
          pgLocks: [ 
             ...
             query: "select l.mode, transactionid, l.granted, " + 
    ...
    def runQuery(params, returnHtml) {
        ...
        def queryData = "${name} : ${localeBundle['executedtime']} ${now() - start}  ${localeBundle['executedtimeunit']}<br/><br/>"
        if (returnHtml) {
	...
	def getMaintenanceOps() {
		def ops = [
			findOrphanedNodes: [
			   ...
			   op: { params ->
				   def output = "<table><tr><th>" + localeBundle['hid'] + "</th><th>" + localeBundle['hType'] + "</th><th>" + localeBundle['hName'] + "</th></tr>"
	...
	def executeMaintenanceOp(params) {
		...
		def maintenanceOpData = "${name}: ${localeBundle['executedtime']} ${now() - start}  ${localeBundle['executedtimeunit']}<br/><br/>"
