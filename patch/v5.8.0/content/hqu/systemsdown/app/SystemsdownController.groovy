...
class SystemsdownController extends BaseController {
    private final DateFormat df = 
        DateFormat.getDateTimeInstance(DateFormat.SHORT, DateFormat.MEDIUM, LocaleSetId.getLocale())
    ...
    private final SYSTEMSDOWN_SCHEMA = [
        ...
        columns: [
            [field:[getValue: {localeBundle.SystemDownResource},
                 description:'SystemDownResource', sortable:true], width:'37%',
             label:{linkTo(it.name,
                           [resource:it.resource.entityId])}],
            [field:[getValue: {localeBundle.SystemDownType},
                 description:'SystemDownType', sortable:true], width:'30%',
             label:{it.type}],
            [field:[getValue: {localeBundle.SystemDownDownSince},
                 description:'SystemDownDownSince', sortable:true], width:'15%',
             label:{df.format(it.timestamp)}],
            [field:[getValue: {localeBundle.SystemDownDownTime},
                 description:'SystemDownDownTime', sortable:true], width:'13%',
             //label:{formatDuration(it.duration)}
              label:{formatDuration(it.duration)}
              ],
            [field:[getValue: {localeBundle.SystemDownAlerts},
                 description:'SystemDownAlerts', sortable:true], width:'5%',
             label:{
