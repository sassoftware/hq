...
public class VerticalChart extends Chart
{
    ...
    protected String[] getXLabels() {
        ...
        for (int i = 0; i < collSize; i++) {
            ...
            result[i] = ScaleFormatter.formatTimeNLS(dp.getTimestamp(),
                                                  this.m_timeScale, collSize, LocaleContextHolder.getLocale());
        }
    ...
    protected int getYLabelWidth(Graphics2D g) {
        FormattedNumber[] fmtValueLabels =
            UnitsFormat.formatSame(m_adRangeMarks, m_fmtType, m_fmtScale, LocaleContextHolder.getLocale());
    ...
    protected Rectangle draw(ChartGraphics g) {
        ...
        int xAvgLabel      = x2 - m_metricsLabel.stringWidth(this.getAverageLabel()) - 3;
        int yAvgLabel      = yAvgLine - 3;
        Rectangle avgLabel = new Rectangle();
        if(this.showAverage)
            avgLabel.setRect(xAvgLabel, yAvgLabel,
                             m_metricsLabel.stringWidth(this.getAverageLabel()),
                             m_metricsLabel.getHeight());

        int xBaselineLabel = x2 - m_metricsLabel.stringWidth(this.getBaselineLabel()) -
                             4;
        ...
        if(this.showBaseline)
            baselineLabel.setRect(xBaselineLabel, yBaselineLabel,
                                  m_metricsLabel.stringWidth(this.getBaselineLabel()),
                                  m_metricsLabel.getHeight());
        
        int xLowLabel      = x2 - m_metricsLabel.stringWidth(this.getLowLabel()) - 4;
        int yLowLabel      = yLowLine - 3;
        Rectangle lowLabel = new Rectangle();
        if(this.showLow)
            lowLabel.setRect(xLowLabel, yLowLabel,
                             m_metricsLabel.stringWidth(this.getLowLabel()),
                             m_metricsLabel.getHeight());
        
        int xPeakLabel      = x2 - m_metricsLabel.stringWidth(this.getPeakLabel()) - 4;
        int yPeakLabel      = yPeakLine - 3;
        Rectangle peakLabel = new Rectangle();
        if(this.showPeak)        
            peakLabel.setRect(xPeakLabel, yPeakLabel,
                              m_metricsLabel.stringWidth(this.getPeakLabel()),
                              m_metricsLabel.getHeight());                                          
        ...
        if(this.showLow) {
            ...
            graph.drawLine(this.xVertMarks, yLowLine, this.x2VertMarks,
                                yLowLine);
            graph.drawString(this.getLowLabel(), xLowLabel, yLowLabel);
        ...
        if(this.showAverage) {
            if(avgLabel.intersects(lowLabel))
              xAvgLabel = xLast - this.m_metricsLabel.stringWidth(this.getAverageLabel()) -
            ...
            graph.drawString(this.getAverageLabel(), xAvgLabel, yAvgLabel);
        ...
        if(this.showPeak) {
            ...
            || peakLabel.intersects(avgLabel))
                xPeakLabel = xLast -
                             this.m_metricsLabel.stringWidth(this.getPeakLabel()) -
                             this.m_metricsLabel.charWidth('W'); 
                
            graph.setColor(this.peakLineColor);
            graph.drawLine(xVertMarks, yPeakLine, x2VertMarks, yPeakLine);
            graph.drawString(this.getPeakLabel(), xPeakLabel, yPeakLabel);
        ...
        if(this.showBaseline && yBaseLine > rect.y && yBaseLine < y2) {
            ...
            || baselineLabel.intersects(peakLabel))
                xBaselineLabel = xLast -
                               this.m_metricsLabel.stringWidth(this.getBaselineLabel()) -
                               this.m_metricsLabel.charWidth('W'); 
            
            graph.setColor(this.baselineColor);
            graph.drawLine(xVertMarks, yBaseLine, x2VertMarks, yBaseLine);
            graph.drawString(this.getBaselineLabel(), xBaselineLabel, yBaselineLabel);
        }
