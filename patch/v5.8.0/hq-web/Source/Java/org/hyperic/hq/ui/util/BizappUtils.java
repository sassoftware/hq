...
public class BizappUtils {
    ...
    public static List<ConfigValues> buildLoadConfigOptions4Secret( String prefix,
            ConfigSchema config,
            ConfigResponse oldResponse)
		{
		List options = config.getOptions();
		...
		while (i < nOptions) {
			...
			if(option instanceof StringConfigOption) {
				if(((StringConfigOption) option).isSecret()) {
					configValue.setIsSecret(true);
					String val = configValue.getValue();
					if(val != null){
						configValue.setValue(EncryptionUtil.encrypt(val));
					}
				}
			}
			...
			// this option use its value and construct , otherwise use an 
			// eppty-string.
			StringTokenizer tok = new StringTokenizer(oldResponse.getValue(option.getName()) != null ?
												     oldResponse.getValue(option.getName()) :
												     ""," ");
			List uiArrayOptions = new ArrayList();
               ...
               InvalidOptionValsFoundException 
    {
        ...
        while(params.hasMoreElements()) {
            ...
            if(stringKeys.contains( shortParam )) {
               ...
               ConfigOption option = config.getOption(shortParam);
               if(option instanceof StringConfigOption && 
                  ((StringConfigOption) option).isSecret()) {
	               if(EncryptionUtil.isMarkedEncryption(value)){
