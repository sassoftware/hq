...
public class NewGroupAction
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
        ...
        if (forward != null) {
        	if(forward.getName().equalsIgnoreCase(Constants.RESET_URL)) {
        		// reset button clicked, recover the saved eid and resource type in the session,
        		// they will be used in showing new group form page.
        	...
        	// Always remove the saved eid and resource type from session.
        	session.removeAttribute(fix_attr_eid);
        	session.removeAttribute(fix_attr_ff);
            return forward;
        }
        // Always remove the saved eid and resource type from session.
    	session.removeAttribute(fix_attr_eid);
